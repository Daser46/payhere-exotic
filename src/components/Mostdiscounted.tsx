...
const DiscountedProducts: React.FC = () => {
    ...
    useEffect(() => {
        const fetchDiscountedProducts = async () => {
            try {
                setLoading(true);
                // Fetch products that might have a discounted_price
                const { data, error } = await supabase
                    .from('products')
                    .select('id, name, category, price, discounted_price, main_image')
                    .not('discounted_price', 'is', null); // Only fetch products where discounted_price is not null

                if (error) throw error;

                if (data) {
                    // Filter for actual discounts and calculate percentage
                    const discounted = data
                        .filter(p => p.discounted_price && p.discounted_price < p.price)
                        .map(p => ({
                            ...p,
                            // Ensure calculation handles potential floating point issues for display
                            discount_percentage: Math.round(((p.price - p.discounted_price) / p.price) * 100),
                            // Assign colors
                            bgColor: '', // Will be assigned during sorting/slicing
                            textColor: '', // Will be assigned during sorting/slicing
                            image: p.main_image,
                            title: p.name,
                    ...
                    // Sort by highest discount percentage and limit to 8
                    const sortedAndLimitedProducts = discounted
    ...
    if (loading) {
        return (
            <div className="container p-4 mx-auto w-full lg:w-5/6 py-16 md:py-20">
                <h1 className='text-3xl md:text-4xl text-center md:text-left mb-8 font-Concert'>Hot Deals & Discounts</h1>
                <div className="flex justify-center items-center h-64">
                    <p>Loading exciting deals...</p>
                </div>
            </div>
        );
    }
    ...
    if (products.length === 0) {
        return (
            <div className="container p-4 mx-auto w-full lg:w-5/6 py-16 md:py-20">
                <h1 className='text-3xl md:text-4xl text-center md:text-left mb-8 font-Concert'>Hot Deals & Discounts</h1>
                <div className="flex justify-center items-center h-64">
                    <p>No exciting discounts available right now. Check back soon!</p>
                </div>
            </div>
        );
    }
                ...
                <AnimatePresence mode='wait'>
                    ...
                    >
                        <Link href={`/Products/${products[currentIndex].id}`}>
                            <div className={`w-full h-[380px] max-w-xs relative overflow-hidden ${products[currentIndex].bgColor} rounded-lg shadow-lg`}>
                                {/* Discount percentage badge */}
                                <span className="absolute top-4 left-4 z-10 bg-white text-green-600 font-bold rounded-full text-sm px-3 py-1">
                                    -{products[currentIndex].discount_percentage}%
                                </span>
                                {/* Price display */}
                                <div className="absolute top-4 right-4 z-10 text-white flex flex-col items-end">
                                    <span className="text-xs line-through opacity-75">LKR {formatPrice(products[currentIndex].price)}</span>
                                    <span className="text-lg font-bold">LKR {formatPrice(products[currentIndex].discounted_price)}</span>
                                </div>
            ...
            <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 place-items-stretch">
                {products.map((product) => (
                    <Link key={product.id} href={`/Products/${product.id}`}>
                        <div className={`w-full h-[380px] max-w-xs relative overflow-hidden ${product.bgColor} rounded-lg shadow-lg`}>
                            {/* Discount percentage badge */}
                            <span className="absolute top-4 left-4 z-10 bg-white text-green-600 font-bold rounded-full text-sm px-3 py-1">
                            ...
                            {/* Price display */}
                            <div className="absolute top-4 right-4 z-10 text-white flex flex-col items-end">
                                <span className="text-xs line-through opacity-75">LKR {formatPrice(product.price)}</span>
                                <span className="text-lg font-bold">LKR {formatPrice(product.discounted_price)}</span>
                            </div>
