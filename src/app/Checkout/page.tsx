...
const CheckoutPage = () => {
    ...
    const handleSubmit = async () => {
        if (validateFormData(formData)) {
            ...
            console.log(orderId)
            if (orderId != -1) {
                let merchantSecret = process.env.NEXT_PAYHERE_SECRET || 'MjA0MDM1ODI4MzIwNjI4Nzc5ODMxNzY5NDU1ODQ5MzQ1MDc1MDQxNQ==';
                let merchantId = process.env.NEXT_MERCHANT_ID;
                let amount = 1000;
                let hashedSecret = md5(merchantSecret).toString().toUpperCase();
                let amountFormated = amount.toFixed(2).toLocaleString().replaceAll(',', '');
                let currency = 'LKR';
